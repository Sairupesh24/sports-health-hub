...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE policyname = 'Allow authenticated users to read exercises' AND tablename = 'exercises'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE policyname = 'Allow admins to modify exercises' AND tablename = 'exercises'
    ) THEN
