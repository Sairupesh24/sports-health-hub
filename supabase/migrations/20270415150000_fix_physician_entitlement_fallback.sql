-- Migration: Update complete_session with Sports Physician fallback
-- 
-- Adds a robust fallback for the 'Sports Physician' role to ensure 
-- that sessions finalized by physicians without an explicit service_id
-- can still attempt to deduct from relevant entitlements.
...
BEGIN
    ...
    IF v_resolved_svc_id IS NULL THEN
        ...
        -- Check profession if role doesn't yield enough context
        IF v_therapist_role IS NULL OR v_therapist_role = 'consultant' THEN
            SELECT profession INTO v_therapist_role
        ...
        ELSIF v_therapist_role = 'Nutritionist' THEN
            ...
            WHERE  organization_id = v_session.organization_id
              ...
              AND  is_active = true
            LIMIT 1;
        ELSIF v_therapist_role = 'Sports Physician' THEN
            SELECT id INTO v_resolved_svc_id
            FROM   public.services
            WHERE  organization_id = v_session.organization_id
              AND  (
