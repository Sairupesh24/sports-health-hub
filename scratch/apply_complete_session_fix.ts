...
BEGIN
    ...
    IF v_resolved_svc_id IS NULL THEN
        -- Safely get profession from profiles (p.role DOES NOT EXIST)
...
async function applyFix() {
    console.log('Applying direct database fix for complete_session...');
    const { error } = await supabase.rpc('exec_sql', { sql_query: SQL });

    if (error) {
        console.error('❌ FAILED to apply fix:', error.message);
    } else {
        console.log('✅ SUCCESS: complete_session function updated in the database.');
    }
}
