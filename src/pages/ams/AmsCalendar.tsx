...
export default function AmsCalendar() {
  ...
  const fetchAthletes = async () => {
    try {
      const profileData = await apiFetch<any[]>('/ams/athletes');
      const batchData = await apiFetch<any[]>('/ams/batches');
      const combined = [
      ...
      console.error("Failed to fetch athletes/batches", error);
    }
  };
  ...
  const fetchAthleteWorkouts = async () => {
    try {
      ...
      if (selectedAthlete.entityType === 'batch') {
        params.batch_id = selectedAthlete.id;
      } else {
        params.athlete_id = selectedAthlete.id;
      }

      const data = await apiFetch<any[]>('/ams/program-assignments', { params });
      setWorkouts(data || []);
    } catch (error) {
  ...
  const handleDeleteWorkout = async (assignmentId: string) => {
    try {
      setLoading(true);
      await apiFetch(`/ams/program-assignments/${assignmentId}`, { method: 'DELETE' });
  ...
  const handleDeleteItem = async (itemId: string, e: React.MouseEvent) => {
    ...
    try {
      setLoading(true);
      await apiFetch(`/ams/workout-items/${itemId}`, { method: 'DELETE' });
  ...
  const handleEditWorkout = (workout: any) => {
    const mappedDays = [{
      id: workout.workout_day_id || workout.id,
  ...
  const handleDeleteDailyWorkouts = async (dayWorkouts: any[]) => {
    ...
    try {
      ...
      for (const w of dayWorkouts) {
        await apiFetch(`/ams/program-assignments/${w.id}`, { method: 'DELETE' });
      }
      fetchAthleteWorkouts();
    } catch (error) {
        ...
        <div className="flex flex-1 overflow-hidden bg-[#F2F4F7]">
          <aside className="w-full sm:w-80 bg-[#1A1F26] text-white flex flex-col border-r border-white/5 shadow-2xl z-20 shrink-0">
            ...
            <div className="p-6 mt-auto">
               <Button onClick={() => setIsAssignModalOpen(true)} className="w-full h-14 bg-primary hover:bg-primary/90 rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-xl shadow-primary/20 gap-3 border-none ring-1 ring-white/10">
          ...
          <div className="flex-1 flex flex-col">
            <header className="h-16 bg-[#FF6B35] flex items-center justify-between px-8 shadow-lg z-10 shrink-0">
                        ...
                        {athletes.filter(a => a.entityType === 'batch').length > 0 && (
                          <CommandGroup heading="Teams & Batches" className="text-[10px] font-black uppercase text-primary border-b border-white/5 py-2 px-3">
                            {athletes.filter(a => a.entityType === 'batch').map((batch) => (
                              <CommandItem key={batch.id} value={`batch ${batch.name}`} onSelect={() => { setSelectedAthlete(batch); setIsAthletePopoverOpen(false); }} className="flex items-center gap-3 p-4 cursor-pointer data-[selected=true]:bg-primary/30 hover:bg-white/10 transition-all text-white border-b border-white/5 last:border-none">
                                ...
                                <BatchInfoPopover batch={batch} onUpdate={fetchAthletes} onManage={() => { setEditingBatch(batch); setIsBatchModalOpen(true); setIsAthletePopoverOpen(false); }} />
                              </CommandItem>
                            ))}
                          </CommandGroup>
                        )}
              ...
              <div className="flex items-center gap-6">
                 <Button onClick={() => setIsBatchModalOpen(true)} className="h-10 bg-white/10 hover:bg-white/20 text-white font-black uppercase tracking-widest text-[10px] rounded-xl border-none ring-1 ring-white/20 gap-2 shadow-lg">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-3 no-scrollbar scroll-smooth">
              {!selectedAthlete ? (
                ...
                timelineDays.map((day, index) => {
                  ...
                  return (
                    <div key={index} ref={(el) => { if (el) dayRefs.current.set(format(day, 'yyyy-MM-dd'), el); }} className="flex gap-4 group">
                      <div className="flex flex-col items-center pt-2">
                        <div className={cn("w-12 h-12 rounded-full flex flex-col items-center justify-center transition-all border-2", isTodayDate ? "bg-primary border-primary text-white shadow-lg shadow-primary/30" : "bg-white border-slate-200 text-slate-400 group-hover:border-primary/40 group-hover:text-primary")}>
                      ...
                      <div className={cn("flex-1 min-h-[140px] bg-white rounded-[2rem] border p-6 flex items-start justify-between transition-all relative overflow-hidden group/card shadow-sm", isTodayDate ? "ring-2 ring-primary/20 border-primary/20 shadow-primary/5" : "border-slate-100 hover:border-slate-300 hover:shadow-md")}>
                        <div className="space-y-4 w-full">
                           {dayWorkouts.length === 0 ? (
                             <>
                               <div className="flex items-center gap-3">
                                 <div className="w-4 h-4 rounded-full border-2 transition-all cursor-pointer border-slate-200" />
                                 <h3 className="text-xl font-black uppercase tracking-tight text-slate-300 italic">Untitled Workout</h3>
                               </div>
                               {!isPastDay && (
                                 <div className="flex items-center gap-2 pl-7 mt-4">
                                    <Button variant="ghost" size="sm" className="bg-[#10B981]/5 hover:bg-[#10B981] text-[#10B981] hover:text-white rounded-xl font-black uppercase text-[10px] tracking-widest h-10 px-4 transition-all gap-2 border-none" onClick={() => { setSelectedAssignDate(day); setIsAssignModalOpen(true); }}>
                                       <div className="w-5 h-5 rounded-full bg-[#10B981]/20 flex items-center justify-center group-hover:bg-white/20"><Plus className="w-3 h-3" /></div> Add Workout
                                    </Button>
                                 </div>
                               )}
                             </>
                               ...
                               {dayWorkouts.map((workout: any, wIdx: number) => (
                                 <div key={wIdx} className="w-full">
                                   <div className="flex items-center gap-3">
                                     ...
                                     <h3 className="text-xl font-black uppercase tracking-tight text-slate-900">{workout.title || "Untitled Workout"}</h3>
                                     <Badge variant="outline" className="ml-auto text-[8px] font-black uppercase shadow-xs border-slate-200 text-slate-500 tracking-widest bg-slate-50">{workout.programName}</Badge>
                                   </div>
                                             ...
                                             return (
                                               <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between group/ext bg-white hover:bg-slate-50/50 p-2 sm:p-0 rounded-xl transition-colors border sm:border-none border-slate-100 mb-2 sm:mb-0">
                                                 <div className="flex items-start gap-4 flex-1 min-w-0 pr-4">
                                                   <div className="w-8 h-8 rounded-full bg-slate-50 flex flex-center justify-center shrink-0 border border-slate-200 shadow-sm relative z-10 group-hover/ext:border-primary/40 group-hover/ext:bg-primary/5 transition-all"><Dumbbell className="w-3.5 h-3.5 text-slate-400 group-hover/ext:text-primary" /></div>
                                                   <div className="flex flex-col min-w-0 pt-0.5">
                                                     <p className="font-bold text-slate-800 text-sm truncate group-hover/ext:text-primary">{exerciseName}</p>
                                                     <p className="text-[11px] text-slate-500 font-semibold tracking-wide mt-0.5">{item.lift_items.sets} x {item.lift_items.reps} {item.lift_items.load_value > 0 ? `@ ${item.lift_items.load_value}kg` : ''}</p>
                                                   </div>
                                                 </div>
                                                 {!isPastDay && (
                                                   <div className="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover/ext:opacity-100 transition-opacity mt-2 sm:mt-0 justify-end pt-2 sm:pt-0">
                                                     <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-blue-500" onClick={(e) => { e.stopPropagation(); handleEditWorkout(workout); }}><Edit className="w-3.5 h-3.5" /></button>
                                                     <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-red-500" onClick={(e) => handleDeleteItem(item.id, e)}><Trash2 className="w-3.5 h-3.5" /></button>
                                                   </div>
                                                 )}
                                               </div>
                                             );
                        ...
                        {!isPastDay && (
                          <div className="flex items-center gap-2">
                             <Popover>
                               <PopoverTrigger asChild>
                                 <button className="p-3 hover:bg-slate-50 rounded-2xl text-slate-300 hover:text-slate-600 transition-all opacity-0 group-hover/card:opacity-100"><MoreVertical className="w-5 h-5" /></button>
                               </PopoverTrigger>
                               <PopoverContent className="w-48 bg-white border-slate-200 rounded-2xl shadow-xl p-2" align="end">
                                  <Button variant="ghost" className="w-full justify-start text-red-500 hover:text-red-600 hover:bg-red-50 h-10 px-3 rounded-xl gap-2 font-bold text-xs uppercase tracking-tight" onClick={() => handleDeleteDailyWorkouts(dayWorkouts)}><Trash2 className="w-4 h-4" /> Delete All for Day</Button>
                                </PopoverContent>
                             </Popover>
                          </div>
                        )}
