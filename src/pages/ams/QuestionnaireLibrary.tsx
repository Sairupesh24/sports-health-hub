...
export default function QuestionnaireLibrary() {
  ...
  const { data: recurringSchedules, refetch: refetchSchedules } = useQuery({
    queryKey: ["ams-recurring-questionnaires", profile?.organization_id],
    queryFn: async () => {
      return await apiFetch<any[]>('/ams/recurring-questionnaires');
    },
    enabled: !!profile?.organization_id
  });
  ...
  const handleToggleScheduleStatus = async (id: string, currentStatus: string) => {
    try {
      await apiFetch(`/ams/recurring-questionnaires/${id}`, {
        method: 'PATCH',
        body: { status: currentStatus === 'active' ? 'paused' : 'active' }
      });
      refetchSchedules();
    } catch (err) {
      console.error(err);
          ...
          <Tabs defaultValue="library" className="w-full">
            <div className="flex items-center justify-between mb-8 overflow-x-auto pb-2 custom-scrollbar">
              <TabsList className="bg-white p-1.5 rounded-2xl border border-slate-200">
                ...
                <TabsTrigger value="schedules" className="rounded-xl px-6 py-2.5 font-black uppercase text-[10px] tracking-widest flex items-center gap-2 data-[state=active]:bg-slate-900 data-[state=active]:text-white transition-all">
                  <Clock className="w-3.5 h-3.5" /> Recurring Schedules
                </TabsTrigger>
              </TabsList>
            </div>
            ...
            <TabsContent value="schedules" className="mt-0">
              <div className="max-w-4xl space-y-6">
                <div className="mb-8">
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight italic">Recurring Schedules</h2>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Monitor squad assessments scheduled to trigger automatically at intervals.</p>
                </div>
                
                {!recurringSchedules || recurringSchedules.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[3rem] border border-dashed border-slate-200">
                    <Clock className="w-12 h-12 text-slate-250 mb-4" />
                    <h3 className="text-lg font-black text-slate-900 uppercase">No schedules active</h3>
                    <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-1">To schedule recurring assessments, select a form from the library and choose the recurring assignment option.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {recurringSchedules.map((rq: any) => (
                      <Card key={rq.id} className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:shadow-md transition-all">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center gap-3">
                            <h3 className="font-black text-xl text-slate-900 uppercase tracking-tight italic">
                              {rq.questionnaire_name || "Assessment Questionnaire"}
                            </h3>
                            <Badge className={cn(
                              "rounded-full px-3 py-0.5 text-[8px] font-black uppercase tracking-wider border-none",
                              rq.status === "active" ? "bg-emerald-50 text-emerald-600" : "bg-slate-150 text-slate-500"
                            )}>
                              {rq.status}
                            </Badge>
                            <Badge className="bg-blue-50 text-blue-600 rounded-full px-3 py-0.5 text-[8px] font-black uppercase tracking-wider border-none">
                              {rq.recurrence_type}
                            </Badge>
                          </div>
                          
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-500 text-[10px] font-bold uppercase tracking-wider">
                            <span className="flex items-center gap-1">
                              <Users className="w-3 h-3 text-primary" /> {rq.client_ids?.length || 0} Athletes
                            </span>
                            <span className="flex items-center gap-1">
                              <Calendar className="w-3 h-3 text-primary" /> Next Run: {rq.next_run ? format(new Date(rq.next_run), "dd MMM yyyy, hh:mm a") : "Pending"}
                            </span>
                            <span className="flex items-center gap-1">
                              <History className="w-3 h-3 text-primary" /> Last Dispatched: {rq.last_run ? format(new Date(rq.last_run), "dd MMM yyyy, hh:mm a") : "Never"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto justify-end border-t md:border-t-0 pt-4 md:pt-0 border-slate-100">
                          <button
                            onClick={() => handleToggleScheduleStatus(rq.id, rq.status)}
                            className="h-10 rounded-xl px-4 font-black uppercase text-[10px] tracking-widest flex items-center gap-2 border border-slate-200 text-slate-650 hover:bg-slate-50 transition-colors"
                          >
                            {rq.status === "active" ? (
                          ...
                          <button
                            onClick={() => handleDeleteSchedule(rq.id)}
                            className="h-10 rounded-xl px-4 font-black uppercase text-[10px] tracking-widest flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white transition-colors"
                          >
