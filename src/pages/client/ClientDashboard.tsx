    ...
    <DashboardLayout role="client">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-display font-bold text-foreground">Welcome, {profile?.first_name || 'Client'}</h1>
            <p className="text-muted-foreground mt-1">Your recovery is progressing well — keep it up!</p>
          </div>
          <Button onClick={() => navigate('/client/book')} className="gradient-primary text-primary-foreground font-semibold w-full sm:w-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-xl border border-border bg-card p-5 gradient-card">
            <div className="flex items-center justify-between mb-4">
              ...
              {appointments.length > 0 && <button onClick={() => navigate('/client/appointments')} className="text-xs text-primary font-medium hover:underline">View all</button>}
            </div>
            <div className="space-y-2">
              {appointments.length === 0 ? (
                <div className="text-center p-4 bg-muted/20 rounded-lg text-sm text-muted-foreground">
                  You have no upcoming appointments mapped. Let's schedule your next session!
                </div>
              ) : (
