...
export function WaitlistSidebar({ selectedDate, onBook }: Props) {
    ...
    return (
        <div
            className={cn(
                "relative flex flex-col h-full bg-white/40 backdrop-blur-md border-l border-border/50 shadow-2xl",
                "transition-all duration-300 ease-in-out overflow-hidden",
                collapsed ? "w-[48px] min-w-[48px]" : "w-[300px] min-w-[300px]"
            )}
        >
            {/* ── Collapsed strip ─────────────────────────────────────────── */}
            {collapsed && (
                ...
                >
                    ...
                    {/* Count bubble — always visible */}
                    <div className={cn(
                        "w-7 h-7 rounded-full flex items-center justify-center shrink-0 border-2 font-black text-xs leading-none",
                        waitlist.length > 0
                    ...
                    {waitlist.length > 0 && (
                        <div className="w-2 h-2 rounded-full bg-primary animate-pulse mb-1 shrink-0" />
                    )}
                </button>
            )}
