...
async function test() {
    const { data: iData, error } = await supabase
        ...
        .select(`
            id,
            client_id,
            client:profiles!injuries_client_id_fkey(
                id, 
                first_name, 
                last_name, 
                assigned_consultant_id,
                consultant:profiles!assigned_consultant_id(first_name, last_name)
            )
        `)
        .limit(1);
    
    fs.writeFileSync('tmp/out_query.json', JSON.stringify({ data: iData, error: error }, null, 2));
}
